  ...
  async function fetchTokens(accountId?: string) {
    ...
    try {
      ...
      const balanceMap = new Map<
        string,
        { balance: string; balanceUsd: number; icon: string | null }
      >();
      ...
      tokensArray = tokensArray.sort((a, b) => {
        ...
        const aVolumeScore =
          a.volume_usd_24h > 10 ? Math.log2(a.volume_usd_24h) : 0;
        const bVolumeScore =
          b.volume_usd_24h > 10 ? Math.log2(b.volume_usd_24h) : 0;
    ...
    update((state) => {
      ...
      const tokens = state.tokens.map((token) => {
        ...
        if (updated) {
          return {
            ...token,
            userBalance: updated.balance,
            userBalanceUsd: updated.balanceUsd,
          };
        }
        return token;
      });
